...
{
    class SqlPermissionManagement : IManageUserPermissions, IPrincipalValidation
    {
        #region Commands
...
", parameters: new Dictionary<string, Action<DbParameter>>
        ...
        public static readonly CommandBuilder GetUsersAndRolesCommand = new CommandBuilder(
            commandText: @"
SELECT [User].[ClaimName]
      ,[Role]
  FROM [User]
  LEFT JOIN [UserRole] ON [User].[ClaimName]=[UserRole].[ClaimName]
");

        public static readonly CommandBuilder EnsureUserCommand = new CommandBuilder(
            commandText: @"
MERGE INTO [User] AS OldUser
USING (SELECT @ClaimName AS ClaimName) AS NewUser
ON NewUser.ClaimName = OldUser.ClaimName
WHEN NOT MATCHED THEN INSERT (ClaimName) VALUES (NewUser.ClaimName);
", parameters: new Dictionary<string, Action<DbParameter>>
            {
                { "@ClaimName", p => p.DbType = System.Data.DbType.AnsiString },
            });
...
", parameters: new Dictionary<string, Action<DbParameter>>
        ...
        public SqlPermissionManagement(IServiceProvider serviceProvider)
        {
